...
from .vesting_schedule import Schedule
...
class StockGrant(Component):
    def __init__(
        ...
        stock_value_date: Optional[datetime.date] = None,
    ):
        if stock is None:
            self.stock = Stock(
                initial_value=1,
        ...
        print(
            f"Creating grant with stock amount: {self.amount} number of stock sis {self.number_of_stocks}"
        )
